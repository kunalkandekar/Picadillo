...
namespace PCADLO {
	 ...
	 * instantiation of PCADLODictionary.
	 */
	...
	public class PCADLODictionary<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary, ISerializable, IDeserializationCallback
#if NET_4_5
...
#endif
	{
		...
		// The number of hash code collisions
		int nHashCollisions;
		...
		public int NumHashCollisions {
			get { return nHashCollisions; }
		}
		...
		public TValue this [TKey key] {
			get {
				...
				while (cur != NO_SLOT) {
					...
					if (linkSlots [cur].HashCode == hashCode && ((linkSlots [cur].CollisionCount == 0) ||  (hcp.Equals (keySlots [cur], key))))
			...
			set {
					...
					do {
						...
						if (linkSlots [cur].HashCode == hashCode) {
						    if (hcp.Equals (keySlots [cur], key))
                                break;
                            cc++;
                            //Console.WriteLine("Collision! "+keySlots[cur]+" and "+key+" collide with hash="+hashCode);
						}
						prev = cur;
						cur = linkSlots [cur].Next;
					} while (cur != NO_SLOT);
				...
				if (cur == NO_SLOT) {
					if(cc > 0) {
					   nHashCollisions++;
					    //track collisions: increment count for all hashcode-matching slots
        				while (recur != NO_SLOT){
    						// The ordering is important for compatibility with MS and strange
    						// Object.Equals () implementations
    						if (linkSlots [recur].HashCode == hashCode) {
                                linkSlots [recur].CollisionCount = cc;
    						}
    						recur = linkSlots [cur].Next;
        				}
					}
					...
					// store the new item and its hash code and its collision count
					linkSlots [cur].HashCode = hashCode;
					linkSlots [cur].CollisionCount = cc;
					keySlots [cur] = key;
				} else {
		...
		public PCADLODictionary ()
		{
			Init (INITIAL_SIZE, null);
		}

		public PCADLODictionary (IEqualityComparer<TKey> comparer)
		{
			Init (INITIAL_SIZE, comparer);
		}

		public PCADLODictionary (IDictionary<TKey, TValue> dictionary)
			: this (dictionary, null)
		{
		}

		public PCADLODictionary (int capacity)
		{
			Init (capacity, null);
		}

		public PCADLODictionary (IDictionary<TKey, TValue> dictionary, IEqualityComparer<TKey> comparer)
		{
			if (dictionary == null)
		...
		public PCADLODictionary (int capacity, IEqualityComparer<TKey> comparer)
		{
			Init (capacity, comparer);
		}

		protected PCADLODictionary (SerializationInfo info, StreamingContext context)
		{
		...
		private void Resize ()
		{
			...
			for (int i = 0; i < table.Length; i++) {
				...
				while (cur != NO_SLOT) {
				    ...
				    newLinkSlots [cur].CollisionCount = linkSlots [cur].CollisionCount;
				    //if(newLinkSlots [cur].CollisionCount > 0) Console.WriteLine("Collision carried over during resize! "+keySlots[cur]+" hash="+linkSlots[cur].HashCode+" colCount="+linkSlots [cur].CollisionCount);
		...
		public bool Remove (TKey key)
		{
			...
			if (linkSlots[cur].CollisionCount > 0) { //update collision counts
    			while (recur != NO_SLOT) {
    				// The ordering is important for compatibility with MS and strange
    				// Object.Equals () implementations
    				if (linkSlots [recur].HashCode == hashCode)
                        linkSlots [recur].CollisionCount--;
    				recur = linkSlots [recur].Next;
    			} 
			}
			...
			linkSlots [cur].CollisionCount = 0;
		...
		{
			PCADLODictionary<TKey, TValue> dictionary;
			int next;
			int stamp;
		...
		public sealed class KeyCollection : ICollection<TKey>, IEnumerable<TKey>, ICollection, IEnumerable {
			...
			public KeyCollection (PCADLODictionary<TKey, TValue> dictionary)
			{
				if (dictionary == null)
					throw new ArgumentNullException ("dictionary");
				this.dictionary = dictionary;
			...
			public struct Enumerator : IEnumerator<TKey>, IDisposable, IEnumerator {
				PCADLODictionary<TKey, TValue>.Enumerator host_enumerator;
